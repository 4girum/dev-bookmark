...
import { db_getNextPage, db_deleteBookmark } from "@/lib/actions/bookmarks";
...
function SkeletonCard() {
  return (
    <div aria-hidden="true" className="flex flex-col gap-3 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
...
interface BookmarkFeedProps {
  ...
  onClearFilters: () => void;
  /** Lift the selected bookmark up to BookmarkShell to open the edit modal. */
  onEdit: (bookmark: Bookmark) => void;
  /** When Shell saves an edit, it passes the updated bookmark here to patch local state. */
  updatedItem?: Bookmark | null;
}
...
export default function BookmarkFeed({
  ...
  // Reset when the server re-renders a fresh initialPage (after revalidation).
  useEffect(() => {
  ...
  // Patch a single item in pages when Shell resolves an edit.
  useEffect(() => {
  ...
  const loadNextPage = useCallback(async () => {
    ...
    try {
      ...
      setPages((prev) => {
        ...
        return [...prev, ...result.items.filter((b) => !existingIds.has(b.id))];
      });
  ...
  async function handleDelete(id: string) {
    setPages((prev) => prev.filter((b) => b.id !== id));
    try {
      await db_deleteBookmark(id);
    } catch {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-4 text-sm font-medium text-neutral-500 dark:text-neutral-400">
          {isFiltering ? "No bookmarks match your search." : "No bookmarks yet — add your first one!"}
        </p>
        {isFiltering && (
          <button type="button" onClick={onClearFilters} className="mt-3 text-sm text-blue-600 hover:underline dark:text-blue-400">
            Clear filters
          </button>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      {filtered.length > 0 && (
        <ul className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3" aria-label="Bookmarks">
      ...
      {loading && (
        <ul aria-label="Loading more bookmarks" className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <li key={`skeleton-${i}`}><SkeletonCard /></li>
          ))}
        </ul>
      )}
      ...
      {error && (
        <div className="flex flex-col items-center gap-2 py-4 text-center">
          ...
          <button type="button" onClick={loadNextPage} className="text-sm text-blue-600 hover:underline dark:text-blue-400">Try again</button>
        </div>
      )}
